...
def set_trainable(model: ks.Model, trainable: Union[str, List[str]], verbose=False):
    """Set trainable layers of a Keras model.

    Note: This function does not freeze the weights of layers with further children (nodes).

    Parameters:
        model : keras.Model
            Model to be modified.
        trainable : str or list of str
            Layer names to be set to trainable. Unix shell-style wildcards can be used.
        verbose : bool
            If True, print information about the layers that are set to trainable.
...
def SubModel(model: ks.Model, layer_name: str, flatten=False) -> ks.Model:
